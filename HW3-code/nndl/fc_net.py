...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    a1 = affine_forward(X,self.params['W1'],self.params['b1'])
    h = relu_forward(a1[0])
    a2 = affine_forward(h[0],self.params['W2'],self.params['b2'])
    scores = a2[0]
    ...
    loss += 1/2 * self.reg * np.sum(self.params['W1']**2) + 1/2 * self.reg * np.sum(self.params['W2']**2)
    
    dx, grads['W2'], grads['b2'] = affine_backward(ds,a2[1])
    _, grads['W1'], grads['b1'] = affine_backward(relu_backward(dx,h[1]),a1[1])
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    loss +=  1/2*self.reg*sum([np.linalg.norm(W,'fro')**2 for W in Ws])
    
    ds_ = {}
    dldh_ = {}
    dldw_ = {}
    dldb_ = {}
    ds_[self.num_layers] = ds

    for i in range(self.num_layers)[::-1]:
        dldh,dldw,dldb = affine_backward(ds_[i+1], a[i+1][1])
        ...
        if i != 0:
            ds = relu_backward(dldh_[i],h[i][1])
            ds_[i] = ds
