...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    h = np.maximum(0, np.dot(W1, X.T)+ np.matrix(b1).T)
    z = np.dot(W2, h)+np.matrix(b2).T
    scores = z.T
    ...
    correctLog = -np.log(p[range(N), y])
    softmax_loss = np.sum(correctLog)/N
    regloss = 1/2 * reg * (np.linalg.norm(W1,'fro')**2 + np.linalg.norm(W2,'fro')**2)
    ...
    p[np.arange(N),y] -= 1
    p /= N
    grads['W2'] = p.T.dot(h.T) + reg * W2
    grads['b2'] = np.sum(p,axis=0)
    
    dh = np.dot(p, W2).T
    dh[h<=0] = 0
    
    grads['W1'] = np.dot(X.T, dh.T).T + reg * W1
    grads['b1'] = np.sum(dh, axis = 1).T
  ...
  def predict(self, X):
    ...
    layer1 = np.maximum(0,np.dot(X,self.params['W1'].T) + self.params['b1'])
    layer2 = np.dot(layer1,self.params['W2'].T) + self.params['b2']
